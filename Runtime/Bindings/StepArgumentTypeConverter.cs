namespace TechTalk.SpecFlow.Bindings
{
    ...
    public class StepArgumentTypeConverter : IStepArgumentTypeConverter
    {
        ...
        public bool CanConvert(object value, IBindingType typeToConvertTo, CultureInfo cultureInfo)
        {
            ...
            IStepArgumentTransformationBinding stepTransformation = GetMatchingStepTransformation(value, typeToConvertTo, false);
            if (stepTransformation != null)
            {
                return true;
            }
        ...
        {
            IStepArgumentTransformationBinding[] stepTransformations = bindingRegistry.GetStepTransformations().Where(t => CanConvert(t, value, typeToConvertTo)).ToArray();
        ...
        {
            Match match = stepTransformation.Regex.Match(stepSnippet);
            IEnumerable<string> argumentStrings = match.Groups.Cast<Group>().Skip(1).Select(g => g.Value);
            IBindingParameter[] bindingParameters = stepTransformation.Method.Parameters.ToArray();
            return argumentStrings
                .Select((arg, argIndex) => Convert(arg, bindingParameters[argIndex].Type, cultureInfo))
                .ToArray();
        }
